...
impl Player {
    ...
    fn collides(&self, assets: &Assets, level: &Level) -> bool {
    ...
    pub(crate) fn update(
        ...
        self.integrate(assets, level, Vec2::new_x(1.0), delta_time);
        self.integrate(assets, level, Vec2::new_y(1.0), delta_time);
        assets.player.draw_tile(camera, self.position, UVec2::ZERO);
    }

    fn integrate(&mut self, assets: &Assets, level: &mut Level, dir: Vec2, delta_time: f32) {
        let motion = self.velocity * dir * delta_time;
        self.position += motion;
        if self.collides(assets, level) {
            self.velocity *= Vec2::new(1.0, 1.0) - dir;
            if motion.y.signum() > 0.0 {
                self.jumps = self.max_jumps;
            }
            while self.collides(assets, level) {
